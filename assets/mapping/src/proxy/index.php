...
define('PATH', '../../images/uploaded/proxy');//relative folder where the images are saved
    ...
    if(
        ...
        if(false !== $h) {
            while(false !== ($f = readdir($h))) {
                if(
                    is_file($p . $f) && is_dir($p . $f) === false && strpos($f, SECPREFIX) !== false && (INIT_EXEC - filectime($p . $f)) > (CCACHE * 2)
                ) {
                    unlink($p . $f);
                }
            }
